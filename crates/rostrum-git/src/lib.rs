...
//!    carrying git's own message. Under the default policy in
//!    [`Repo::rebase_onto`] the conflict has already been aborted, so the
//!    repository really is as it was.
...
//! pure function over a string git actually printed — [`parse_status_v2`],
//! [`parse_ab`], [`parse_left_right_count`], [`in_progress`],
//! [`classify_run`], [`classify_fetch`], [`parse_fetch_porcelain`],
//! [`blockers`], [`BranchName::new`], [`Oid::parse`] — so the interesting cases
//! are literals in a test rather than a fixture repository on disk.
